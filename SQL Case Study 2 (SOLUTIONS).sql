...
--Q1--BEGIN 
select customer_name,state,year(date) as year from dim_location dl
inner join fact_transactions ft on dl.idlocation=ft.idlocation
inner join dim_customer c on ft.idcustomer=c.idcustomer
...
--Q2--BEGIN
	select top 1  country,state,Manufacturer_Name,count(Manufacturer_Name)as maxsales  from DIM_LOCATION dl
	inner join fact_transactions ft on dl.idlocation=ft.idlocation
	inner join dim_model dm on ft.idmodel=dm.idmodel
	inner join dim_manufacturer man on dm.idmanufacturer=man.IDManufacturer
...
--Q3--BEGIN      
select IDModel,zipcode,state,count(IDModel) as numtransactions from FACT_TRANSACTIONS ft
...
--Q4--END
---5.Find out the average price for each model in the top5 manufacturers interms of sales quantity and order by average price.
--Q5--BEGIN
SELECT top 5 Manufacturer_Name, model_name, 
COUNT(Quantity) as Total_Quantity, AVG(unit_price) as AVG_Price from FACT_TRANSACTIONS FT
...
--Q6--BEGIN
select customer_name,Date,avg(totalprice) as amount_spent from DIM_CUSTOMER dc
left join  FACT_TRANSACTIONS ft on dc.IDCustomer=ft.IDCustomer
...
--Q8--BEGIN
select * from(select ROW_NUMBER() OVER(order by sum(totalprice) desc) as ronum,manufacturer_name,
sum(TotalPrice) as totalsales from DIM_MANUFACTURER dm
left join DIM_MODEL md on dm.IDManufacturer=md.IDManufacturer
left join FACT_TRANSACTIONS ft on md.IDModel=ft.IDModel
where year(date)='2009'
...
--Q9--BEGIN
select manufacturer_name from (select manufacturer_name from DIM_MANUFACTURER dmr
inner join DIM_MODEL dm on dmr.IDManufacturer=dm.IDManufacturer
inner join FACT_TRANSACTIONS ft on dm.IDModel=ft.IDModel
where year(date)='2010'
except
select manufacturer_name from DIM_MANUFACTURER dmr
inner join DIM_MODEL dm on dmr.IDManufacturer=dm.IDManufacturer
inner join FACT_TRANSACTIONS ft on dm.IDModel=ft.IDModel
...
--Q10--BEGIN
	WITH CTE AS (SELECT TOP 100 ROW_NUMBER() OVER(ORDER BY DATE) AS RONUM,DATE,DC.IDCustomer,AVG(QUANTITY) AS QTY,AVG(TOTALPRICE) AS SPEND,
	CUSTOMER_NAME FROM DIM_CUSTOMER DC
	LEFT JOIN FACT_TRANSACTIONS AS FT ON DC.IDCustomer=FT.IDCustomer
	GROUP BY DATE,CUSTOMER_NAME,DC.IDCustomer)
	SELECT T2.RONUM,T2.DATE,T2.QTY,T2.SPEND,T2.IDCUSTOMER,
	(T2.SPEND-T3.SPEND)*1.0/T2.SPEND*100 AS [PERCENTAGE OF CHANGE IN SPEND]
	FROM CTE AS T2
	LEFT JOIN CTE T3 ON T2.RONUM=T3.RONUM+1
